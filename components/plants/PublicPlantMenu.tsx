...
import { MoreVertical, AlertCircle, Save } from 'lucide-react';
import { toast } from 'sonner';
import { wishlistApi } from '@/lib/api/wishlist';
import { Plant, Genus, Variety } from '@/lib/api';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
interface PublicPlantMenuProps {
  plant: Plant;
  userId: string;
  username: string;
}

export function PublicPlantMenu({ plant, userId, username }: PublicPlantMenuProps) {
  ...
  const getGenusId = () => {
    return typeof plant.genusId === 'string'
      ? plant.genusId
      : (plant.genusId as Genus)?._id || '';
  };

  const getVarietyId = () => {
    return plant.varietyId
      ? typeof plant.varietyId === 'string'
        ? plant.varietyId
        : (plant.varietyId as Variety)?._id
      : undefined;
  };
  ...
  const addToWishlist = async () => {
    ...
    try {
      ...
      const photoUrl = plant.photo
        ? `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3008/api'}/plants/photo/${plant.photo}`
        : null;

      let photoFile: File | null = null;
      if (photoUrl) {
        photoFile = await copyPhotoToFile(photoUrl, `wishlist_${plant._id}_${Date.now()}.jpg`);
      }
